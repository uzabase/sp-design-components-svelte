...
<ul class="segmentedControl__base">
  ...
  <li class="segmentedControl__item" data-testid="segmentedControlItem">
    <label>
      <input data-testid="segmentedControlRadio" class="segmentedControl__radio" type="radio" value="{item.value}" name="{name}" checked="{item.value === value}" disabled="{item.disabled}" on:change={()=> handleOnChange(item.value)}>
